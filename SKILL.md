...
⚠️ **各 Phase 中直接引用問卷答案（下方 `ANS_X`），不可重新詢問使用者。**

| 代號 | 問題 | 預設值 | 使用於 |
|:----:|:-----|:------:|:-------|
| ANS_1 | **影片主題是什麼？**（e.g. 太宰府旅遊 vlog） | — | Phase 1 專案名稱、Phase 8 輸出檔名 |
| ANS_2 | **素材在哪個資料夾？**（e.g. `~/Desktop/太宰府影片/`） | — | Phase 1 `import_media`；路徑衍生：輸出→`{ANS_2}/{ANS_1}_精華版.mp4`，暫存→`{ANS_2}/temp/` |
| ANS_3 | **剪輯風格？**（A: 流水帳 / B: AI挑重點 / C: 自訂腳本） | 流水帳 | Phase 3 決定走 A、B 或 C |
| ANS_4 | **主要語言？** | zh-TW | Phase 7 Whisper `--language` |
| ANS_5 | **影片比例與解析度？**（比例：16:9 / 9:16 / 1:1 / 4:3；解析度：720p / 1080p / 2K / 4K） | 16:9 1080p | Phase 1 `new_project` |
| ANS_6 | **音樂需求？**（僅風格 B 時詢問；A 跳過）— 自備或上網抓 CC0、風格偏好、音量預設 -20dB、淡入淡出預設 2s | — | Phase 6 BGM 選擇 |
| ANS_7 | **輸出編碼？** | H.264 | Phase 8 `export_project` |

⚠️ Agent 必須記憶以上 7 個答案，在後續對應 Phase 使用，不得重新問使用者。
...
```
Step 1. new_project(name="ANS_1", aspectRatio=ANS_5的比例, quality=ANS_5的解析度, fps=30)
...
**C：自訂腳本模式**
```
Step 7c. 產生腳本草稿 → 使用者修改 → 依腳本剪輯：
         a) 對每支 A-roll 素材 call inspect_media(mediaRef=ID, maxFrames=3)
            取得畫面抽幀 + ASR 摘要
         b) get_transcript() 取得逐字稿（含 clipId 對應的檔案 hash）
         c) 產生腳本檔 <TEMP_DIR>/腳本_<主題>.txt：
            格式參考桌面/長崎/transcript_腳本格式.txt：
            ══════════════════════════════════════
              主題名稱 · 逐字稿（分段排版）
            ══════════════════════════════════════
            ...
            ...
            規則：
            - 🎬 後接素材的檔案 hash（來自分類時的 inspect_media），
              讓 script parsing 可以對應到 mediaRef
            - Chapter 標題留空讓使用者填（【Chapter N】）
            - 保留完整原始逐字稿行，不做摘要或刪減
            - Agent 不可自行決定保留或刪除任何段落
         d) 告知使用者：「腳本已產生在 <TEMP_DIR>/腳本_<主題>.txt，
            請編輯 Chapter 標題、刪除不要的段落，完成後通知我繼續。」
         e) 等待使用者回應（腳本編輯完成）
         f) 讀取使用者的腳本，逐行解析：
            - 🎬 FILE_HASH  → 找到對應的 mediaRef
            - Chapter 分界 → 定義章節 frame 範圍（給 Phase 6 BGM 用）
            - 被使用者刪除的行 → 該段落不加入 timeline
            - 被使用者保留的行 → 計算累積 startFrame 上 timeline
         g) 依腳本內容建構時間軸（進入 Phase 4）
```
...
word-level 時間戳 SRT。語言參數使用 ANS_4（問卷收集的語言）。
...
Step 33. export_project(
           mode="video", codec="ANS_7", resolution=ANS_5的解析度,
           outputPath="<ANS_2>/<ANS_1>_精華版.mp4"
         )
         或
         若 ANS_7 == "fcpxml"：
         export_project(
           mode="fcpxml",
           outputPath="<ANS_2>/<ANS_1>_精華版.fcpxml"
         )
         → fcpxml 可匯入 DaVinci Resolve / Final Cut Pro
         ⚠️ outputPath 不可省略（預設會跑到 ~/Downloads/）
